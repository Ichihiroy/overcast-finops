...
- **TLS**: create a `letsencrypt-prod` ClusterIssuer (cert-manager itself is
  already running). DNS already exists: Terraform binds a **static public IP
  with an Azure DNS label** to the ingress controller
  (`infra/terraform/platform.tf`), so `terraform output ingress_fqdn` gives a
  stable hostname (`thelocals-ironhack-prod.westeurope.cloudapp.azure.com`) —
  no domain purchase needed, and it is re-claimed identically after the
  nightly wipe.
...
Set the repo variable `FRONTEND_HOST_PRODUCTION` to the `ingress_fqdn` output
(staging can stay host-less — it then answers on the bare IP). The hosts flow
into the Argo CD Application specs as `TF_VAR`s, so run one more infra PR →
gated apply for them to take effect.
